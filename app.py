...
# ── Ask page ───────────────────────────────────────────────────────────────────
ASK_HTML = """<!DOCTYPE html><html lang="en"><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Ask Claude — DailyPulse</title>""" + CSS + """</head><body>
""" + NAV.replace('{MA}','').replace('{PA}','').replace('{LA}','').replace('{CA}','').replace('{NA}','').replace('{AA}','active').replace('{DA}','') + """
<div class="wrap">
<div class="key-row">
  <label>Anthropic API Key:</label>
  <input type="password" class="kinput" id="ak" placeholder="sk-ant-api03-…">
  <button class="btn btn-g" style="padding:7px 14px;font-size:12px" onclick="saveKey()">Save</button>
...
<div class="suggs">
  <span class="sugg">What are today's biggest market risks?</span>
  <span class="sugg">Which stocks look oversold or near a breakout?</span>
  <span class="sugg">How do current geopolitical events affect energy prices?</span>
  <span class="sugg">Summarize today's national security headlines</span>
  <span class="sugg">What policy changes could move markets this week?</span>
  <span class="sugg">Explain RSI and MACD signals in plain English</span>
</div>
<div class="chat-wrap">
  <div class="chat-hist" id="ch">
    <div data-placeholder style="text-align:center;color:var(--muted);font-size:13px;margin:auto;padding:20px">
      Ask anything about markets, geopolitics, or investments.<br>
      <span style="font-size:11px">Powered by Claude. Add your Anthropic API key above — get one free at console.anthropic.com</span>
    </div>
  </div>
  <div class="input-row">
    <textarea class="tinput" id="ci" rows="2" placeholder="Ask a question… (Enter to send, Shift+Enter for newline)"></textarea>
    <div style="display:flex;flex-direction:column;gap:6px">
      <button class="btn btn-p" id="sb" onclick="send()">Send</button>
      <button class="btn btn-g" onclick="clearChat()">Clear</button>
    </div>
  </div>
</div>
</div>
...
async function loadKey(){
  ...
  if(c.anthropic_key) document.getElementById('ak').value=c.anthropic_key;
}
async function saveKey(){
  const k=document.getElementById('ak').value.trim();
  await fetch('/api/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({anthropic_key:k})});
  toast('API key saved ✓');
}
function toast(msg){
  const t=document.createElement('div');
  t.style.cssText='position:fixed;bottom:20px;right:20px;background:#3fb950;color:#fff;padding:8px 16px;border-radius:6px;font-size:13px;z-index:999';
  t.textContent=msg; document.body.appendChild(t); setTimeout(()=>t.remove(),2500);
}
function addMsg(role,html){
  const ch=document.getElementById('ch');
  if(ch.querySelector('[data-placeholder]')) ch.innerHTML='';
  const d=document.createElement('div'); d.className=`msg ${role}`;
  d.innerHTML=`<div class="role">${role}</div><div class="bubble">${html}</div>`;
...
async function send(){
  ...
  const thinking=addMsg('assistant','<span style="color:var(--muted)">Thinking…</span>');
  try{
    const res=await fetch('/api/ask',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({question:q,history:hist.slice(0,-1),key})});
    const data=await res.json();
    thinking.querySelector('.bubble').innerHTML=
      data.error?`<span style="color:var(--neg)">${data.error}</span>`:
      data.answer.replace(/</g,'&lt;').replace(/\\n/g,'<br>');
    if(data.answer) hist.push({role:'assistant',content:data.answer});
  }catch(e){thinking.querySelector('.bubble').innerHTML='<span style="color:var(--neg)">Connection error</span>';}
  sb.disabled=false;
}
function clearChat(){hist=[];const ch=document.getElementById('ch');ch.innerHTML=
  '<div style="text-align:center;color:var(--muted);font-size:13px;margin:auto;padding:20px" data-placeholder>Ask anything about markets, geopolitics, or investments.</div>';}
document.getElementById('ci').addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send();}});
document.querySelectorAll('.sugg').forEach(s=>s.addEventListener('click',()=>{
  document.getElementById('ci').value=s.textContent; document.getElementById('ci').focus();
}));
loadKey();
</script>
</body></html>
...
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Portfolio — DailyPulse</title>""" + CSS + """</head><body>
""" + NAV.replace('{MA}','').replace('{PA}','active').replace('{LA}','').replace('{CA}','').replace('{NA}','').replace('{AA}','').replace('{DA}','') + """
<div class="wrap">

<div class="sum-row" style="grid-template-columns:repeat(auto-fill,minmax(200px,1fr))" id="sum-row">
  ...
  <div class="tbl-wrap">
    <table>
      <thead><tr>
        ...
        <th>Value</th><th>P&amp;L</th><th>P&amp;L%</th><th>Today</th><th>Allocation</th><th></th>
      </tr></thead>
      <tbody id="port-tbody"><tr><td colspan="10" class="loading">Loading…</td></tr></tbody>
    </table>
  </div>
</div>

<div class="sec">
  <div class="sec-title">Add Holding</div>
  <div class="form-row">
    <div class="form-field" style="max-width:90px"><label>Symbol</label><input class="form-input" id="p-sym" placeholder="AAPL" style="text-transform:uppercase"></div>
    <div class="form-field" style="max-width:100px"><label>Shares</label><input class="form-input" id="p-shares" placeholder="10" type="number" min="0" step="any"></div>
    <div class="form-field" style="max-width:110px"><label>Avg Cost/Share</label><input class="form-input" id="p-cost" placeholder="150.00" type="number" min="0" step="any"></div>
    <button class="btn btn-p" onclick="addHolding()" style="align-self:flex-end">Add</button>
  </div>
</div>
</div>
...
function fp(v){if(v==null)return'—';return v>=1000?'$'+v.toLocaleString('en',{minimumFractionDigits:2,maximumFractionDigits:2}):'$'+v.toFixed(2);}
function fpct(v,showSign=true){if(v==null)return'—';const s=showSign&&v>=0?'+':'';return`<span class="${v>=0?'pos':'neg'}">${s}${v.toFixed(2)}%</span>`;}
function fval(v,showSign=true){if(v==null)return'—';const s=showSign&&v>=0?'+':'';return`<span class="${v>=0?'pos':'neg'}">${s}${fp(Math.abs(v))}</span>`;}
...
async function loadPortfolio(){
  ...
  portData=d;
  // Summary cards
  const sr=document.getElementById('sum-row');
  if(!d.holdings||!d.holdings.length){
    sr.innerHTML='<div class="sum-card" style="grid-column:1/-1;text-align:center;color:var(--muted)">No holdings yet. Add your first position below.</div>';
    document.getElementById('port-tbody').innerHTML='<tr><td colspan="10" class="loading">No holdings</td></tr>';
    return;
  }
  const dayColor=d.day_pnl>=0?'var(--pos)':'var(--neg)';
  const pnlColor=d.total_pnl>=0?'var(--pos)':'var(--neg)';
  sr.innerHTML=`
    <div class="sum-card"><div class="sum-label">Total Value</div><div class="sum-val">$${d.total_value.toLocaleString('en',{minimumFractionDigits:2,maximumFractionDigits:2})}</div><div class="sum-sub" style="color:var(--muted)">Invested: ${fp(d.total_cost)}</div></div>
    <div class="sum-card"><div class="sum-label">Total P&amp;L</div><div class="sum-val" style="color:${pnlColor}">${d.total_pnl>=0?'+':''}${fp(Math.abs(d.total_pnl))}</div><div class="sum-sub" style="color:${pnlColor}">${d.total_pnl_pct>=0?'+':''}${d.total_pnl_pct.toFixed(2)}% all time</div></div>
    <div class="sum-card"><div class="sum-label">Today's P&amp;L</div><div class="sum-val" style="color:${dayColor}">${d.day_pnl>=0?'+':''}${fp(Math.abs(d.day_pnl))}</div><div class="sum-sub" style="color:var(--muted)">${d.holdings.length} position${d.holdings.length>1?'s':''}</div></div>`;
  // Holdings table
  document.getElementById('port-tbody').innerHTML=d.holdings.map(h=>`
    <tr>
      <td style="font-family:'SF Mono',monospace;font-size:13px;font-weight:700">${h.symbol}</td>
      <td>${h.shares}</td>
      <td>${fp(h.cost)}</td>
      <td>${fp(h.price)}</td>
      <td>${fp(h.value)}</td>
      <td>${fval(h.pnl)}</td>
      <td>${fpct(h.pnl_pct)}</td>
      <td>${fpct(h.day_pct)}</td>
      <td>
        <div style="font-size:12px;color:var(--muted);margin-bottom:3px">${h.allocation}%</div>
        <div class="alloc-bar" style="width:${Math.min(100,h.allocation*2)}px"></div>
      </td>
      <td><button onclick="removeHolding('${h.symbol}')" style="background:rgba(255,61,107,.12);border:1px solid rgba(255,61,107,.3);color:var(--neg);border-radius:5px;padding:2px 8px;cursor:pointer;font-size:11px">✕</button></td>
    </tr>`).join('');
}
...
async function addHolding(){
  ...
  if(!sym||isNaN(shares)||isNaN(cost)||shares<=0||cost<=0){alert('Fill in symbol, shares, and cost');return;}
  const curr=portData.holdings||[];
  const existing=curr.findIndex(h=>h.symbol===sym);
  const cfg=await fetch('/api/config').then(r=>r.json());
  const port=cfg.portfolio||[];
  if(existing>=0){port[existing]={symbol:sym,shares,cost};}else{port.push({symbol:sym,shares,cost});}
  await fetch('/api/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({portfolio:port})});
...
async function removeHolding(sym){
  const cfg=await fetch('/api/config').then(r=>r.json());
  const port=(cfg.portfolio||[]).filter(h=>h.symbol!==sym);
  await fetch('/api/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({portfolio:port})});
  loadPortfolio();
}
...
# ── Alerts page ─────────────────────────────────────────────────────────────────
ALERTS_HTML = """<!DOCTYPE html><html lang="en"><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Alerts — DailyPulse</title>""" + CSS + """</head><body>
""" + NAV.replace('{MA}','').replace('{PA}','').replace('{LA}','active').replace('{CA}','').replace('{NA}','').replace('{AA}','').replace('{DA}','') + """
<div class="wrap">

<div class="sec" id="triggered-sec" style="display:none">
  <div class="sec-title" style="color:var(--pos)">&#9888; Triggered Alerts</div>
  <div id="triggered-list"></div>
</div>

<div class="sec">
  <div class="sec-title">All Alerts</div>
  <div id="alerts-list"><div class="loading">Loading alerts…</div></div>
</div>

<div class="sec">
  <div class="sec-title">Add Alert</div>
  <div class="form-row">
    <div class="form-field" style="max-width:90px"><label>Symbol</label><input class="form-input" id="a-sym" placeholder="AAPL" style="text-transform:uppercase"></div>
    <div class="form-field" style="max-width:130px"><label>Target Price</label><input class="form-input" id="a-target" placeholder="200.00" type="number" min="0" step="any"></div>
    <div class="form-field" style="max-width:130px"><label>Condition</label>
      <select class="form-input" id="a-cond">
        <option value="above">Rises Above ↑</option>
        <option value="below">Falls Below ↓</option>
      </select>
    </div>
    <div class="form-field"><label>Note (optional)</label><input class="form-input" id="a-note" placeholder="e.g. breakout level"></div>
    <button class="btn btn-p" onclick="addAlert()" style="align-self:flex-end">Add Alert</button>
  </div>
</div>
</div>
...
async function loadAlerts(){
  ...
  const triggered=data.filter(a=>a.triggered);
  const ts=document.getElementById('triggered-sec');
  const tl=document.getElementById('triggered-list');
  const al=document.getElementById('alerts-list');
  if(triggered.length){
    ts.style.display='block';
    tl.innerHTML=triggered.map(a=>`
      <div class="alert-card alert-triggered">
        <span class="alert-sym" style="color:var(--pos)">${a.symbol}</span>
        <span class="alert-status status-triggered">&#10003; TRIGGERED</span>
        <span style="font-size:13px">${a.condition==='above'?'↑ Rose above':'↓ Fell below'} ${fp(a.target)}</span>
        <span style="font-size:13px;margin-left:auto">${fp(a.price)} <span class="${a.pct>=0?'pos':'neg'}">(${a.pct>=0?'+':''}${a.pct.toFixed(2)}%)</span></span>
        ${a.note?`<span style="font-size:11px;color:var(--muted)">${a.note}</span>`:''}
      </div>`).join('');
  } else { ts.style.display='none'; }
  if(!data.length){
    al.innerHTML='<div class="loading">No alerts set. Add one below.</div>'; return;
  }
  al.innerHTML=data.map(a=>`
    <div class="alert-card ${a.triggered?(a.condition==='above'?'alert-triggered':'alert-triggered neg-alert'):''}">
      <span class="alert-sym">${a.symbol}</span>
      <span class="alert-status ${a.triggered?'status-triggered':'status-watching'}">${a.triggered?'✓ HIT':'● WATCHING'}</span>
      <span style="font-size:12px;color:var(--muted)">${a.condition==='above'?'↑ above':'↓ below'}</span>
      <span style="font-size:13px;font-weight:600">${fp(a.target)}</span>
      <span style="font-size:12px;color:var(--muted)">→</span>
      <span style="font-size:13px">${a.price!=null?fp(a.price):'—'}</span>
      ${a.price!=null?`<span class="${a.distance>=0?'pos':'neg'}" style="font-size:12px">(${a.distance>=0?'+':''}${a.distance.toFixed(1)}% away)</span>`:''}
      ${a.note?`<span style="font-size:11px;color:var(--muted);margin-left:4px">${a.note}</span>`:''}
      <button onclick="removeAlert(${a.id})" style="margin-left:auto;background:rgba(255,61,107,.1);border:1px solid rgba(255,61,107,.25);color:var(--neg);border-radius:5px;padding:2px 8px;cursor:pointer;font-size:11px">✕</button>
    </div>`).join('');
}
...
async function addAlert(){
  ...
  if(!sym||isNaN(target)||target<=0){alert('Enter a valid symbol and target price');return;}
  const cfg=await fetch('/api/config').then(r=>r.json());
  const alerts=[...(cfg.alerts||[]),{symbol:sym,target,condition:cond,note}];
  await fetch('/api/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({alerts})});
  document.getElementById('a-sym').value='';document.getElementById('a-target').value='';document.getElementById('a-note').value='';
  loadAlerts();
}

async function removeAlert(id){
  const cfg=await fetch('/api/config').then(r=>r.json());
  const alerts=(cfg.alerts||[]).filter((_,i)=>i!==id);
  await fetch('/api/config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({alerts})});
  loadAlerts();
}
